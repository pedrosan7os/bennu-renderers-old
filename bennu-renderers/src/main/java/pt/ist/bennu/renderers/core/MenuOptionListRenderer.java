...
/**
 * This renderer as a purpose similar to {@link pt.ist.bennu.renderers.core.CheckBoxOptionListRenderer} but is intended to collect
 * only one value. All the possible values for the slot beeing edited are presented in an html menu. The presentation of each
 * object must have in consideration that the object is beeing presented in an option of the menu so it must be short and simple.
 * If possible used the <tt>format</tt> property to format the object. Nevertheless the usual configuration is possible with
 * <tt>eachLayout</tt> and <tt>eachSchema</tt>.
 * 
 * <p>
...
public class MenuOptionListRenderer extends SelectionRenderer {
	...
	/**
	 * The layout to be used when presenting each object. This property will only be used if {@link #setFormat(String) format} is
	 * not specified.
	 * 
	 * @property
	 */
