...
import pt.ist.bennu.renderers.filters.RequestWrapperFilter.FenixHttpServletRequestWrapper;
import pt.ist.bennu.renderers.servlets.commons.UploadedFile;

/**
 * The standard renderers request processor. This processor is responsible for handling any viewstate present in the request. It
 * will parse the request, retrieve all viewstates, and start the necessary lifecycle associated with them before continuing with
 * the standard struts processing.
 * 
 * <p>
...
public class RenderersRequestProcessorImpl {
	...
	static ThreadLocal<ServletContext> currentContext = new ThreadLocal<>();

	static ThreadLocal<Map<String, UploadedFile>> fileItems = new ThreadLocal<>();
