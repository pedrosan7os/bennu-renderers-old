...
/**
 * The base class for all renderers. Allows all renderers to be configured with the common properties of all html elements:
 * {@link #setClasses(String) classes}, {@link #setStyle(String) style}, and {@link #setTitle(String) title}.
 * 
 * Renderers are currently divided in a complete and disjunctive hierarchy. This class provides the base behaviour for every
 * renderer.
 * 
...
public abstract class AbstractRenderer {
	...
	/**
	 * Allows you to specify the css classes that are to be applied to the top level html element that is generated.
	 * 
	 * @property
	 */
	...
	/**
	 * This property allows you to specify the value of the style attribute of the top level html element that is generated.
	 * 
	 * @property
	 */
	...
	/**
	 * Main method of a renderer that is invoked to do the renderization of a value into a component. The default behaviour is to
	 * {@linkplain #getLayout(Object, Class) get the layout} for this renderer, configure it with the
	 * {@linkplain Layout#getPropertyNames() declared properties} and {@linkplain #renderComponent(Layout, Object, Class) issue
	 * the rendering}.
	 * 
	...
	/**
	 * This is the base method that should be overriden but every renderer. Each renderer must render the given value accordingly
	 * with a layout. The layout represents the way information will be presented.
	 * 
	...
	/**
	 * Copies all propreties from this renderer to the given layout. The layout declares the properties it's interested through
	 * {@link Layout#getPropertyNames()}. If this renderer has a property with the same name, it's copied to the layout.
	 * 
	 * @param the
	 *            layout ot configure
	 */
	...
	/**
	 * Starts the rendering process for a concrete value. The presentation context used to present the value will be a subcontext
	 * of the {@linkplain #getContext() current context}. This means that the presentation will made in the same mode. The
	 * subcontext will be configured with the given schema, layout, and properties.
	 * <p>
	...
	/**
	 * This method is a convenience method for the previous one. It assumes that no properties are set in the subcontext.
	 */
	...
	/**
	 * Starts the rendering process for the value of a meta slot. A subcontext will be created to present the slot's value. That
	 * subcontext will be configured with the layout and properties defined in the slot.
	 */
	...
	/**
	 * Same as previous method but does the rendering with the specified renderer. The renderer must know how to handle the value
	 * of the slot.
	 */
	protected HtmlComponent renderSlot(AbstractRenderer renderer, MetaSlot slot) {
		PresentationContext newContext = createPresentationContext(slot);
