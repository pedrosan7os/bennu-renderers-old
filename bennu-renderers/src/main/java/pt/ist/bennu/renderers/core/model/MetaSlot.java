...
/**
 * A MetaSlot is an abstraction of a slot's value of a concrete domain object. The meta slots provides an interface to present and
 * manipulate that slot by the user without changing the domain until it's really neaded. The meta slot also allows to propagete
 * user inserted values through out the interface.
 * 
 * @author cfgi
 */
public class MetaSlot extends MetaObject {
	...
	/**
	 * This method allows you to obtain a localized label that can be presented in the interface. This label is obtained by using
	 * the provided key and bundle for this slot or the default naming conventions based on the type of the slot's object and this
	 * slot's name.
	 * 
	...
	/**
	 * The meta slot does not change the real slot until the commit is issued. This method allows you to check if a value was
	 * already setted for this meta slot.
	 * 
