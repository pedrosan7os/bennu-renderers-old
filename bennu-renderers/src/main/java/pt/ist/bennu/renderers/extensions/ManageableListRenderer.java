...
/**
 * This renderer allows you get a list as input but delegates part of the list's management to an external action. Each one of the
 * slot's values will be presented in an html list. The renderer manages the removal of values from the list. So a delete action
 * link will appear next to each object's presentation. An add action link will allow the user to start the external
 ...
 * @author cfgi
 */
@Renderer(mode = RenderMode.INPUT, type = List.class, layout = "external-add")
public class ManageableListRenderer extends InputRenderer {
	...
	/**
	 * The destination for were control will be forwarded whn pressing the add button. The destiny action is responsible for
	 * keeping the current <tt>viewstate</tt> during the interaction. The action is also responsible for changing the viewstate to
	 * reflect the intended changes, that is, adding a new value to the managed list, before sending the control back to the
	 * inicial page.
	 * 
	 * <p>
