...
/**
 * This renderer provides a basic presentation for an object. The object is presented in a table that contains it's details. The
 * table will have two columns and a row for each one of the object's slots. In the left column of each row the slot's label will
 * be placed. In the right column the slot's value will be presented.
 * 
 * <p>
 * Example:
 * <table border="1">
...
@Renderers({
		@Renderer(
				layout = "tabular",
				type = Object.class,
				properties = { @RendererProperty(name = "labelTerminator", value = ":") }),
		@Renderer(type = Object.class, properties = { @RendererProperty(name = "labelTerminator", value = ":") }) })
public class StandardObjectRenderer extends OutputRenderer {
	...
	/**
	 * The classes to be used for each column in the generated table. See {@link CollectionRenderer#setColumnClasses(String)}.
	 * 
	 * @property
	 */
	...
	/**
	 * The classes to be used for each row in the table. See {@link CollectionRenderer#setRowClasses(String)}.
	 * 
	 * @property
	 */
	...
	/**
	 * Chooses the suffix to be added to each label. If the label already contains that suffix then nothing will be added.
	 * <p>
