...
/**
 * This input renderer allows several slots to be edited without any special formating. Each slot's editor is placed after the
 * previous. You should note that this technique does not work well for more than two or three slots. You can decreese the space
 * taken by suppresing the slots labels but then you need other way of identifing wich slot is beeing edited.
 * 
 * <p>
 ...
 * @author cfgi
 */
@Renderer(mode = RenderMode.INPUT, layout = "flow", type = Object.class, properties = { @RendererProperty(
		name = "labelTerminator",
		value = ":") })
public class FlowInputRenderer extends InputRenderer {
	...
	/**
	 * Allows to specify the class attribute for each one of the slot's presentation.
	 * 
	 * @property
	 */
	...
	/**
	 * This property selects if each slot should be presented as an inline element or as a block element. By default slots are
	 * presented inline, that is, they will be added inside <tt>span</tt> elements. If <tt>eachInline</tt> is <tt>false</tt> then
	 * a <tt>div</tt> will be used.
	 * 
	 * @property
	 */
	...
	/**
	 * Allows to specify the class of the element around the generated label (when shown)
	 * 
	 * @property
	 */
