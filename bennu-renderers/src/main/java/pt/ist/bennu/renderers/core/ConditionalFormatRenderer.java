...
/**
 * This renderer allows a format string to be used conditionally, depending on the rendered object's value
 * 
 * @author jcrn
 */
public class ConditionalFormatRenderer extends OutputRenderer {
	...
	/**
	 * The format is associated with a condition that indicates if this format can be used for this slot.
	 * 
	 ...
	 * 
	 * If both parameters are specified, the condition is true if useFormatIf is TRUE _AND_ useFormatIfNot is FALSE.
	 * 
	 * If none of the parameters are specified, the condition is always TRUE.
	 * 
	 * When more than one format can be used for this slot, the one with the lowest order is used.
	 */
	public static class ConditionalFormat {
		...
		/**
		 * The format is simple string with a few rules to indicate where slot values will be placed. An example string is
		 * 
		 * <pre>
		 * &quot;${name} is ${age} years old&quot;
		 * </pre>
		 * 
		 * Assuming that we are taking about an object that represents a person and has a <code>getName()</code> and
		 * <code>getAge()</code> methods this format would produce something like: <code>"Jane Doe is 20 years old"</code>
		 * <p>
	...
	private class FormatLayout extends Layout {
		...
		public HtmlComponent createComponent(Object object, Class type) {
			...
			for (ConditionalFormat format : orderedFormats) {
				...
				if (useFormatIfResult == null && useFormatIfNotResult == null || (useFormatIfResult == null || useFormatIfResult)
						&& (useFormatIfNotResult == null || !useFormatIfNotResult)) {
