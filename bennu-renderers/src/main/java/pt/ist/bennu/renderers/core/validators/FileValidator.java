...
/**
 * This validator can be used to validate uploaded files. It allows you to restrict the file maximum size, extension, and type.
 * 
 * @author cfgi
 */
public class FileValidator extends HtmlValidator {
	 ...
	 * specified then it has priority over the accepted types. If the file has no extension then the accepted types are used if
	 * provided.
	 */
	...
	/**
	 * The maximum size allowed for files. You can specify the units using the suffix <code>"MB"</code>, <code>"KB"</code>, or
	 * <code>"B"</code>. If no units are given the value is considered to be in bytes.
	 */
