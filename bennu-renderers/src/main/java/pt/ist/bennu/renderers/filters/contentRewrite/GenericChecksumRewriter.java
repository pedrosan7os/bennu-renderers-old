package pt.ist.bennu.renderers.filters.contentRewrite;
...
public class GenericChecksumRewriter extends RequestRewriter {
	...
	private static String calculateChecksum(final TreeSet<String> strings) {
		...
		final String digest = UserView.getPrivateConstantForDigestCalculation();
		if (digest != null) {
	...
	private boolean isRedirectRequest(final HttpServletRequest httpServletRequest) {
		...
		final int offset = ConfigurationManager.getAppContext().length() + 1;
	...
	protected boolean isPrefixed(final StringBuilder source, final int indexOfTagOpen) {
		return indexOfTagOpen >= LENGTH_OF_NO_CHECKSUM_PREFIX
				...
				&& match(source, indexOfTagOpen - LENGTH_OF_NO_CHECKSUM_PREFIX_HAS_CONTEXT_PREFIX, indexOfTagOpen,
						NO_CHECKSUM_PREFIX_HAS_CONTEXT_PREFIX);
